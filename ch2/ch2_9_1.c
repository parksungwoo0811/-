...
int main() {
    ...
    long loc, loc3; // 위치 정보를 저장할 변수를 선언합니다.
    int pos; // 현재 읽고 있는 파일의 위치를 나타내는 변수를 선언합니다.
    ...
    while ((dent = readdir(dp))) {
        ...
        loc = telldir(dp); // 현재 디렉토리의 위치를 저장합니다.
        printf("Cur Position : %ld\n", loc); // 현재 디렉토리의 위치를 출력합니다.
        pos++; // 파일 위치를 증가시킵니다.
        if (pos == 3) loc3 = loc; // 세 번째 파일의 위치를 저장합니다.
    }

    // 디렉토리 포인터를 처음으로 되감아서 위치를 출력합니다.
    printf("** Directory Position Rewind **\n");
    rewinddir(dp);
    printf("Cur Position : %ld\n", telldir(dp)); // 처음 위치를 출력합니다.

    // 디렉토리 포인터를 이동하여 위치를 출력합니다.
    printf("** Move Directory Pointer **\n");
    seekdir(dp, loc3); // 세 번째 파일의 위치로 이동합니다.
...
이 코드는 디렉토리 내의 파일을 읽고, 디렉토리 포인터를 제어하는 방법을 보여줍니다. 처음에는 디렉토리의 시작 위치를 출력하고, 파일을 하나씩 읽어가며 현재 위치를 출력합니다. 그 후 디렉토리 포인터를 되감고 세 번째 파일의 위치를 저장하고, 해당 위치로 이동하여 현재 위치를 출력합니다. 마지막으로 파일을 읽고 해당 파일의 이름을 출력합니다.